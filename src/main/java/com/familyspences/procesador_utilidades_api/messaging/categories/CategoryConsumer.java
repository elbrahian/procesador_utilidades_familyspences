package com.familyspences.procesador_utilidades_api.messaging.categories;

import com.familyspences.procesador_utilidades_api.domain.categories.Category;
import com.familyspences.procesador_utilidades_api.service.categories.CategoryService;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.amqp.rabbit.annotation.RabbitListener;
import org.springframework.stereotype.Component;

import java.util.Map;

@Component
public class CategoryConsumer {

    private static final Logger log = LoggerFactory.getLogger(CategoryConsumer.class);
    private final CategoryService categoryService;

    public CategoryConsumer(CategoryService categoryService) {
        this.categoryService = categoryService;
        log.info("========================================");
        log.info("CategoryConsumer INITIALIZED!");
        log.info("========================================");
    }

    @RabbitListener(queues = "q.category.create")
    public void handleCategoryCreate(Category category) {
        log.info("Received Category CREATE event: {}", category);
        try {
            categoryService.saveFromProducer(category);
            log.info("Category saved successfully: {}", category.getId());
        } catch (Exception e) {
            log.error("Error processing Category CREATE event. Category ID: {}. Error: {}",
                    category.getId(), e.getMessage(), e);
        }
    }

    @RabbitListener(queues = "q.category.update")
    public void handleCategoryUpdate(Category category) {
        log.info("Received Category UPDATE event: {}", category);
        try {
            categoryService.updateFromProducer(category);
            log.info("Category updated successfully: {}", category.getId());
        } catch (Exception e) {
            log.error("Error processing Category UPDATE event. Category ID: {}. Error: {}",
                    category.getId(), e.getMessage(), e);
        }
    }

    @RabbitListener(queues = "q.category.delete")
    public void handleCategoryDelete(Map<String, String> data) {
        log.info("Received Category DELETE event: {}", data);
        try {
            categoryService.deleteFromProducer(data);
            log.info("Category deleted successfully: {}", data.get("categoryId"));
        } catch (Exception e) {
            log.error("Error processing Category DELETE event. Data: {}. Error: {}",
                    data, e.getMessage(), e);
        }
    }
}